...
class MetaResultadoResourceIT {
    ...
    private MetaResultado insertedMetaResultado;
    ...
    @AfterEach
    public void cleanup() {
        if (insertedMetaResultado != null) {
            metaResultadoRepository.delete(insertedMetaResultado);
            insertedMetaResultado = null;
        }
    }
    ...
    void createMetaResultado() throws Exception {
        ...
        var returnedMetaResultadoDTO = om.readValue(
            restMetaResultadoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(metaResultadoDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            MetaResultadoDTO.class
        );

        // Validate the MetaResultado in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedMetaResultado = metaResultadoMapper.toEntity(returnedMetaResultadoDTO);
        assertMetaResultadoUpdatableFieldsEquals(returnedMetaResultado, getPersistedMetaResultado(returnedMetaResultado));

        insertedMetaResultado = returnedMetaResultado;
    }
    ...
    void getAllMetaResultados() throws Exception {
        ...
        insertedMetaResultado = metaResultadoRepository.saveAndFlush(metaResultado);
        ...
        restMetaResultadoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, metaResultadoDTO.getId())
                    ...
                    .content(om.writeValueAsBytes(metaResultadoDTO))
            )
            .andExpect(status().isOk());

        // Validate the MetaResultado in the database
        assertSameRepositoryCount(databaseSizeBeforeUpdate);
        assertPersistedMetaResultadoToMatchAllProperties(updatedMetaResultado);
    }
    ...
    void putWithMissingIdPathParamMetaResultado() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restMetaResultadoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(metaResultadoDTO)))
    ...
    void partialUpdateMetaResultadoWithPatch() throws Exception {
        ...
        partialUpdatedMetaResultado.parcial(UPDATED_PARCIAL).metaAtingida(UPDATED_META_ATINGIDA).analise(UPDATED_ANALISE);

        restMetaResultadoMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, partialUpdatedMetaResultado.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(partialUpdatedMetaResultado))
            )
            .andExpect(status().isOk());
        ...
        assertSameRepositoryCount(databaseSizeBeforeUpdate);
        assertMetaResultadoUpdatableFieldsEquals(
            createUpdateProxyForBean(partialUpdatedMetaResultado, metaResultado),
            getPersistedMetaResultado(metaResultado)
        );
    }
    ...
    void fullUpdateMetaResultadoWithPatch() throws Exception {
        ...
        assertSameRepositoryCount(databaseSizeBeforeUpdate);
        assertMetaResultadoUpdatableFieldsEquals(partialUpdatedMetaResultado, getPersistedMetaResultado(partialUpdatedMetaResultado));
    }
    ...
    void patchWithMissingIdPathParamMetaResultado() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restMetaResultadoMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(metaResultadoDTO)))
            .andExpect(status().isMethodNotAllowed());
    ...
    protected long getRepositoryCount() {
        return metaResultadoRepository.count();
    }
    ...
    protected MetaResultado getPersistedMetaResultado(MetaResultado metaResultado) {
        return metaResultadoRepository.findById(metaResultado.getId()).orElseThrow();
    }

    protected void assertPersistedMetaResultadoToMatchAllProperties(MetaResultado expectedMetaResultado) {
        assertMetaResultadoAllPropertiesEquals(expectedMetaResultado, getPersistedMetaResultado(expectedMetaResultado));
    }

    protected void assertPersistedMetaResultadoToMatchUpdatableProperties(MetaResultado expectedMetaResultado) {
        assertMetaResultadoAllUpdatablePropertiesEquals(expectedMetaResultado, getPersistedMetaResultado(expectedMetaResultado));
    }
}
