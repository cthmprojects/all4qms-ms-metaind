 ...
 */
@RestController
@RequestMapping("/api/meta-anexos")
public class MetaAnexoResource {

    private static final Logger log = LoggerFactory.getLogger(MetaAnexoResource.class);
     ...
     */
    @PostMapping("")
    public ResponseEntity<MetaAnexoDTO> createMetaAnexo(@RequestBody MetaAnexoDTO metaAnexoDTO) throws URISyntaxException {
        log.debug("REST request to save MetaAnexo : {}", metaAnexoDTO);
        if (metaAnexoDTO.getId() != null) {
            throw new BadRequestAlertException("A new metaAnexo cannot already have an ID", ENTITY_NAME, "idexists");
        }
        metaAnexoDTO = metaAnexoService.save(metaAnexoDTO);
        return ResponseEntity.created(new URI("/api/meta-anexos/" + metaAnexoDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, metaAnexoDTO.getId().toString()))
            .body(metaAnexoDTO);
    }
    ...
    public ResponseEntity<MetaAnexoDTO> updateMetaAnexo(
        ...
        metaAnexoDTO = metaAnexoService.update(metaAnexoDTO);
